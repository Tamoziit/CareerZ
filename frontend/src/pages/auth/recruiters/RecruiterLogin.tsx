...
const RecruiterLogin = () => {
  ...
  const { recruiterLogin, loading } = useRecruiterLogin();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    await recruiterLogin(inputs);
  }

  return (
    <div className="flex flex-col gap-3 items-center justify-center min-h-screen w-full">
      <h1 className="text-[30px] md:text-[35px] lg:text-[40px] gradient-text-1">Login</h1>
        ...
        <div className="flex overflow-hidden">
          ...
          <form className="flex flex-col gap-7 items-start justify-center glassmorphic p-4 w-[320px] md:w-[380px] lg:w-[450px]" onSubmit={handleSubmit}>
            ...
            <div className="flex -mt-7 w-full items-center justify-center">
              <Link to="/applicant/signup" className="text-gray-400 hover:text-blue-600">Don't have an Account? Signup</Link>
            </div>
          </form>
        </div>
      </div>
